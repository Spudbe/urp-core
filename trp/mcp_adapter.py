"""MCP adapter for TRP ToolReceipts.

Provides utilities to attach TRP ToolReceipts to MCP CallToolResult responses
...
to preserve through subsequent requests. TRP uses it to carry a serialised
...
Typical server-side usage (wrapping a tool call)::
    ...
    from trp.mcp_adapter import wrap_mcp_tool_result

    # Your tool function
    output = compute_fibonacci({"n": 10})

    # Wrap into a dict shaped like CallToolResult with TRP receipt in _meta
...
Typical client-side usage (extracting receipt from a result)::
    ...
    from trp.mcp_adapter import extract_tool_receipt
...
# The key used in CallToolResult._meta to carry the TRP receipt.
TRP_META_KEY = "trp:tool_receipt"
...
def wrap_mcp_tool_result(
    ...
    *,
    ...
    """Create an MCP CallToolResult-shaped dict with a TRP receipt in _meta.
    ...
    serialised into ``_meta["trp:tool_receipt"]``.
    ...
    return {
        ...
        "_meta": {TRP_META_KEY: receipt.to_dict()},
        "isError": False,
    }


def extract_tool_receipt(meta: Optional[dict]) -> Optional[ToolReceipt]:
    """Extract a TRP ToolReceipt from MCP CallToolResult._meta.

    Args:
        meta: The ``_meta`` dict from a CallToolResult. May be None.

    Returns:
        A ToolReceipt if one was found under the ``trp:tool_receipt`` key,
    ...
    if meta is None:
        return None
    receipt_data = meta.get(TRP_META_KEY)
