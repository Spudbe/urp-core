...
- **v0.6 features:** All implemented (RFC 8785 JCS, Claim.create(), remote tool replay, EvidenceBundle with signing, CLI, REST API, conformance vectors, examples, Getting Started guide, PyPI prep)
...
| `trp/signing.py` | Ed25519 JWS signing: key generation, detached signatures, receipt signing, evidence bundle signing, message envelope signing |
...
|------|-------|----------------|
| test_core.py | 50 | All protocol types, enums, serialisation round-trips, structured_claim, EvidenceBundle with signing/scope |
...
| test_signing.py | 37 | Key generation, detached JWS, receipt signing, bundle signing, envelope signing |
...
| test_conformance.py | 3 | Conformance test vectors: canonical JSON, receipt hashes, claim matching |
