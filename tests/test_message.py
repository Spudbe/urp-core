import json
import pytest
from trp.core import Claim, ClaimType, ProofReference, Stake
from trp.message import TRPMessage, PROTOCOL_VERSION
...
class TestTRPMessage:
    ...
    def test_from_json_round_trip(self):
        original = TRPMessage("claim", _make_claim(), "agent-1")
        raw = original.to_json(compact=True)
        restored = TRPMessage.from_json(raw, payload_cls=Claim)
    ...
    def test_from_json_mismatched_version_still_parses(self):
        ...
        original = TRPMessage("claim", _make_claim(), "agent-1", protocol_version="99.0.0")
        raw = original.to_json(compact=True)
        restored = TRPMessage.from_json(raw, payload_cls=Claim)
